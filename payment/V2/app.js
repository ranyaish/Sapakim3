...
const STORAGE_KEY = 'payrollSessionV6'; // ↑ גרסה חדשה (סעיפים פר-חודש)
...
// ===================== Extras (per month) helpers =====================
function zeroExtras(){ return {travel:0,tips:0,bonus:0,advance:0}; }
function ensureEmpConfig(emp){
  emp = normEmpName(emp);
  if(!empConfig[emp]) empConfig[emp] = {mode:'A', rate:0, extras:{}}; // extras: { 'YYYY-MM': {..}, __default?:{..} }
...
function aggregateExtrasByEmp(){
  // מחזיר מפת {emp:{travel,tips,bonus,advance}}
  const m = {};
  for(const [emp, cfg] of Object.entries(empConfig)){
    const agg = {travel:0,tips:0,bonus:0,advance:0};
    const ex = cfg.extras || {};
    for(const [k,v] of Object.entries(ex)){
      if(k==='__default') continue;
      agg.travel += +(v.travel||0);
      agg.tips   += +(v.tips||0);
      agg.bonus  += +(v.bonus||0);
      agg.advance+= +(v.advance||0);
    }
    // אם אין כלליים לפי חודש, ננסה __default
    if(Object.values(agg).every(v=> v===0) && ex.__default){
      agg.travel += +(ex.__default.travel||0);
      agg.tips   += +(ex.__default.tips||0);
      agg.bonus  += +(ex.__default.bonus||0);
      agg.advance+= +(ex.__default.advance||0);
    }
    m[normEmpName(emp)] = agg;
  }
  return m;
}
...
function summarizeByEmployee(perDayRows, rateByEmp){
  ...
  // הוספת סכומי סעיפים מצטברים לכל עובד
  const aggExtras = aggregateExtrasByEmp();
  for(const [emp, row] of byEmp.entries()){
    const ex = aggExtras[emp] || zeroExtras();
...
let punches = [];
let perDayBase = [];
// empConfig: { [emp]: { mode:'A'|'B', rate:number, extras: { 'YYYY-MM': {travel,tips,bonus,advance}, __default? } } }
let empConfig = {};
...
function reviveSession(obj){
  ...
  for(const [k,v] of Object.entries(obj.empConfig||{})){
    ...
    // התאמה בין גרסאות: אם קיבלנו extras כאובייקט של ערכים מספריים (גרסה ישנה) נשמור כ-__default
    if(v.extras && (typeof v.extras.travel === 'number' || typeof v.extras.tips === 'number' || typeof v.extras.bonus === 'number' || typeof v.extras.advance === 'number')){
      cfg.extras.__default = {
        travel:+(v.extras.travel||0), tips:+(v.extras.tips||0), bonus:+(v.extras.bonus||0), advance:+(v.extras.advance||0)
      };
    }
    // אם הגיע במבנה החדש (לפי חודשים)
...
function updateOutputs(){
  ...
  renderSummary(summarizeByEmployee(perDayWithPay, mapConfigRate(empConfig)));
...
$('#exportSummary').onclick = ()=>{
  ...
  const rateMap = mapConfigRate(empConfig);
  const rows = summarizeByEmployee(perDay, rateMap).map(r=> ({
...
function loadExtrasIntoForm(emp, ym){
  const ex = ym ? getExtras(emp, ym) : zeroExtras();
...
function updateModalTotals(){
  if(!currentEmpInModal || !currentMonthKey){ $('#empWgt').textContent='0'; $('#empBase').textContent='0'; $('#empExtrasSum').textContent='0'; return; }
...
function updateModalFinal(){
  ...
  const final = base + exSum;
  $('#empFinal').textContent = fmt2(final);
}
['extra_travel','extra_tips','extra_bonus','extra_advance'].forEach(id=>{
  document.addEventListener('input', (e)=>{ if(e.target && e.target.id===id) { updateModalTotals(); updateModalFinal(); } });
});
